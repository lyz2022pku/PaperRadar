...
SYSTEM_PROMPT_TEMPLATE = """{user_intro}
...
A 直接相关性（严格依据关键词等级判断）：
5分：论文主要贡献直接涉及核心关键词，且与{user_ref}的研究高度吻合
4分：论文研究内容涉及核心关键词，但非主要贡献，或方向略有偏差
3分：仅涉及扩展关键词，或核心关键词仅在背景/对比中被提及
2分：与关键词所在领域相关，但不涉及任何关键词
1分：与{user_ref}研究领域几乎无关
...
C 视野拓展性：
5分：来自相邻领域，方法/结论可直接启发{user_ref}的研究思路
4分：提供了新视角或跨领域联系，有间接参考价值
3分：拓宽了对领域整体进展的了解
2分：信息量有限，参考价值较小
1分：与{user_ref}研究背景差距过大，无参考意义
...
def build_system_prompt(core_keywords: List[str], broad_keywords: List[str],
                        user_name: str = "", user_profile: str = "") -> str:
    ...
    if name and profile:
        user_intro = f"你是{name}的学术论文筛选助手。{name}是{profile}。"
        user_ref = name
    elif profile:
        user_intro = f"你是一位{profile}的学术论文筛选助手。"
        user_ref = "用户"
    else:
        user_intro = "你是一位研究者的学术论文筛选助手。"
        user_ref = "用户"
...
def analyze_papers_with_kimi(papers: List[Dict],
                              ...
                              base_url: str = None,
                              user_name: str = "",
                              user_profile: str = "") -> List[Dict]:
    if not papers:
        return []

    client = OpenAI(api_key=api_key, base_url=base_url) if base_url else OpenAI(api_key=api_key)
    system_prompt = build_system_prompt(core_keywords, broad_keywords, user_name, user_profile)
    analyzed = []
    total = len(papers)
